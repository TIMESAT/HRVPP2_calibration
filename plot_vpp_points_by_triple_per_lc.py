...
def parse_args():
    ap = argparse.ArgumentParser(
        description=(
            "For each lc_id, scatter the actual VPP_SCORE points for top-K "
            "(VI, method_name, seasonmethod) combos, colored by seapar. "
            "Order x-axis by best seapar median per combo, and mark the highest single point."
        )
    )
    ...
    ap.add_argument("--seapar-col", default="seapar", help="Column used for coloring points (default: seapar).")
    ap.add_argument("--lc-col", default="lc_id", help="Land cover column (default: lc_id).")
    ap.add_argument("--min-per-group", type=int, default=3, help="Min rows per combo (default: 3).")
    ap.add_argument("--top-k", type=int, default=20, help="Max number of combos to show (default: 20).")
    ap.add_argument("--annotate-setting", action="store_true", help="Include Setting in annotation if available.")
    ap.add_argument("--jitter", type=float, default=0.15, help="Horizontal jitter (default: 0.15).")
    ap.add_argument("--alpha", type=float, default=0.7, help="Point transparency (default: 0.7).")
    ap.add_argument("--markersize", type=float, default=22, help="Point size (default: 22).")
    ap.add_argument("--dpi", type=int, default=220)
    ap.add_argument("--width", type=float, default=14.0)
    ap.add_argument("--height", type=float, default=6.0)
    ap.add_argument("--outdir", default="figures_points_by_triple_bestsea", help="Output directory.")
...
def main():
    ...
    for lc_val, df_lc in df.groupby(args.lc_col):
        ...
        # --- identify single best point for this LC ---
        ...
        # --- new RANKING logic: best seapar median per combo ---
        counts = df_lc.groupby(combo_col)[args.score_col].size()
        eligible = counts[counts >= args.min_per_group].index.tolist()

        per_combo_sea = (
            df_lc.groupby([combo_col, args.seapar_col])[args.score_col].median()
        )
        best_sea_median_per_combo = per_combo_sea.groupby(level=0).max()

        candidates = (
            best_sea_median_per_combo
            .loc[best_sea_median_per_combo.index.intersection(eligible)]
        ...
        for sea in seacats:
            ...
            xj = x + rng.uniform(-args.jitter, args.jitter, size=len(x))
        ...
        if x_best is not None:
            ax.plot([x_best], [best_score], marker="*", markersize=16,
            ...
            ax.annotate(label, xy=(x_best, best_score), xytext=(6, 10),
