...
def read_table_data(df, p_a):
    ...
    wm3[0, 0, :] = df.iloc[:, 2].T.values.astype(np.float32)
...
def _ts_run_(csvfilename, output_file_name, output_png_name):
    ...
    data = pd.DataFrame({"Date": daily_timestep})
    ...
    p_seasonmethod_options = {1: [1, 2], 2: [1]}  # Depends on p_fitmethod
    p_seapar_options = {1: [0.1, 0.3, 0.5, 0.7, 0.9], 2: [1]}  # Depends on p_fitmethod
    ...
    for fitmethod_value in p_fitmethod_options:
        ...
        p_smooth_options = p_smooth_options_method_1 if fitmethod_value == 1 else p_smooth_options_method_2
        for p_smooth_val in p_smooth_options:
            p_smooth.fill(p_smooth_val)  # Ensure p_smooth is also in the correct shape
            
            for seasonmethod_value in p_seasonmethod_options[fitmethod_value]:
                p_seasonmethod.fill(seasonmethod_value)  # Assign the same shape

                for seapar_value in p_seapar_options[fitmethod_value]:
                    ...
                    data = pd.concat([data, yfit_df], axis=1)
